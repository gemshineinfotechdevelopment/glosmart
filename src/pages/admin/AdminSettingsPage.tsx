...
const AdminSettingsPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'profile' | 'footer'>('footer');
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          profile: {
            adminName: data.profile?.adminName || '',
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto pb-24">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              <h2 className="text-3xl font-extrabold text-[#1c1c28] mb-1">Academy Settings</h2>
              <p className="text-slate-500 font-medium">Manage profile details and public contact information.</p>
            </div>
            <button
          ...
          <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.02)] border border-slate-50 p-6 md:p-8">
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              ...
              {activeTab === 'profile' && (
                <div className="animate-[slideUp_0.2s_ease-out]">
                  <h3 className="text-xl font-bold text-[#1c1c28] mb-5 pb-2 border-b border-slate-100 flex items-center gap-2">
                    <FiUser className="text-[#6247df]" /> Admin Profile
                  </h3>
                  ...
                  <div className="flex flex-col gap-6 w-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div>
                        <label className="block text-sm font-bold text-[#1c1c28] mb-2">Admin Name</label>
                        <input
                          type="text"
                          name="adminName"
                      ...
                      <div>
                        <label className="block text-sm font-bold text-[#1c1c28] mb-2">Admin Account Email</label>
                        <input
                          type="email"
                          name="adminEmail"
                          ...
                          onChange={handleProfileChange}
                          disabled={!isEditingProfile}
                          placeholder="admin@glosmart.com"
                          className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-[#6247df] focus:ring-2 focus:ring-purple-100 transition-all font-medium disabled:bg-slate-50 disabled:text-slate-500"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-[#1c1c28] mb-2">Admin Phone Number</label>
                        <input
                          type="text"
                          name="adminPhone"
